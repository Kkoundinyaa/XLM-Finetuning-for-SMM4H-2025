...
# --- Load Data (all languages version) ---
train_df = pd.read_csv("/users/PAS2836/krishnakb/ondemand/krishna_proj/train_data_SMM4H_2025_Task_1.csv")
dev_df = pd.read_csv("/users/PAS2836/krishnakb/ondemand/krishna_proj/dev_data_SMM4H_2025_Task_1.csv")
...
class WeightedThresholdTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # --- Auto-tuned class weights based on label frequency ---
        # counts = Counter(train_df_balanced["label"].tolist())
        # total = sum(counts.values())
        # neg_weight = total / counts[0]
        # pos_weight = total / counts[1]
        # scale = (neg_weight + pos_weight)
        # neg_weight /= scale
        # pos_weight /= scale

        # class_weights = torch.tensor([neg_weight, pos_weight]).to(logits.device)

        # --- Manual Grid Search Option (uncomment to test) ---
        class_weights = torch.tensor([1.0, 1.8]).to(logits.device)
...
print("\n\U0001F9BE Final Evaluation Metrics:\n")
