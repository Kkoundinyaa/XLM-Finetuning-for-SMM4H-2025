...
pred_csv = "/users/PAS2836/krishnakb/ondemand/krishna_proj/Task1/submission_dev_xlm-v.csv"  # output from train_xlmv.py

# Load and merge
true_df = pd.read_csv(dev_csv)[["id", "label", "language"]]
pred_df = pd.read_csv(pred_csv)[["id", "predicted_label", "language"]]
df = pd.merge(true_df, pred_df, on=["id", "language"], how="inner")
...
for lang in df['language'].unique():
    ...
    plt.ylabel("Actual")
    plt.title(f"Confusion Matrix - {lang} (XLM-V)")
    os.makedirs("confusion_matrices_xlmv", exist_ok=True)
    plt.savefig(f"confusion_matrices_xlmv/confusion_matrix_{lang}.png")
    plt.close()

# Save metric table
metrics_df = pd.DataFrame(metrics_summary, columns=["Language", "Accuracy", "Precision", "Recall", "F1", "F1_macro"])
metrics_df.to_csv("language_metrics_xlmv.csv", index=False)
print("✅ Saved language-wise metrics to language_metrics_xlmv.csv")

# Plot loss curves
for lang in df['language'].unique():
    log_path = f"logs_{lang}_xlmv/log_history.pkl"
    ...
    plt.xlabel("Steps")
    plt.ylabel("Loss")
    plt.title(f"Loss Curve - {lang} (XLM-V)")
    plt.legend()
    plt.grid(True)
    os.makedirs("loss_curves_xlmv", exist_ok=True)
    plt.tight_layout()
    plt.savefig(f"loss_curves_xlmv/loss_curve_{lang}.png")
    plt.close()

print("✅ Confusion matrices and loss curves saved for XLM-V.")